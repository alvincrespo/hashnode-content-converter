...
// ============================================================================
// These tests use REAL filesystem operations (not mocked) to verify the
// complete end-to-end conversion pipeline works correctly in flat output mode.
// ============================================================================

// Import os module for temp directory creation
...
describe('convertAllPosts - Flat Output Mode Integration', () => {
  ...
  beforeEach(() => {
    // 1. Restore real fs module for integration tests
    vi.doUnmock('node:fs');

    // 2. Create unique temp directory for isolation
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'hashnode-flat-test-'));

    // 3. Setup nested output structure (required for flat mode validation)
    outputDir = path.join(tempDir, 'blog', '_posts');
    fs.mkdirSync(outputDir, { recursive: true });

    // 4. Create real Converter instance
    // Note: ImageDownloader will use real implementation but we'll avoid
    // actual network calls by using posts without images for most tests
    converter = new Converter();

    // 5. Export path (will be created per-test with specific data)
  ...
  // Integration tests will be added here in subsequent commits
});
